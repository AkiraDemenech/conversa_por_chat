...
class main:
	...
	def __init__ (self):
		...
		self.main.title('UDP chat (File transfer)') # mudando o título da janela principal 
		...
		try:	
			...
			with protocol() as server:
				...
				while self.active:
					...
					msg, address = server.recvfrom(SIZE)
					
					if not address in self.connections:
						self.connections[address] = chat(self.main, server, address, 'from')	 						
					self.connections[address].mainloop(msg)	
	...
	def client (self):		
		...
		try:
			...
			self.connections[address] = chat(self.main, self.socket, address, 'to')
...
class chat:
	...
	def sendall (self, bytes):
		c = 0
		while c < len(bytes):
			c += self.connection.sendto(bytes[c:], self.address)
	...
	def send_burst_ack (self, file, package):		
		...
		self.sendall(('/-a/' + hex(len(self.files[file]['data']) - 1)[2:] + '/' + hex(package)[2:] + '/\\').zfill(self.size).encode())
